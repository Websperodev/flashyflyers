...
<input type="hidden" name="page_id" id="page_id" value="1">
	<input type="text" value="<?php if(basename($_SERVER['SCRIPT_NAME'])=="search.php"){ echo str_replace("%20"," ",$_REQUEST['search_term']);}?>" onkeydown="if (event.keyCode == 13) {jQuery('#searchTopBtn').trigger('click'); return false; }" placeholder="What are you looking for? E.g. Party flyer" name="search_term" id="search_term" />
...
<script>
   function filter_products_search(){$(".mainLoader").show();var formData=$('#filter_form_search_top').serialize();$.ajax({type:"POST",url:"<?=SITEURL;?>ajax/advance-product-filter.php",data:formData,success:function(regResponse){console.log(regResponse);regResponse=JSON.parse(regResponse);$("#productList").html(regResponse.html);
